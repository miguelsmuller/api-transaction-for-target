...
O projeto `Transaction API` é parte do monorepo `Transaction Challenge` e tem como objetivo fornecer uma API RESTful para gerenciar transações de acordo com as regras de negócio fornecidas pela Target. Esta API é desenvolvida em .NET e é uma parte crucial do sistema que permite a interação com o frontend Angular para listar, inserir, alterar e cancelar lançamentos na conta corrente.
...
3. Adicionado os pacotes necessários:
...
2. Executada a migração do banco de dados:
...
O projeto oferece uma [Documentação sSwagger](http://localhost:5028/swagger/)
...
- A API utiliza o banco de dados SQLite como mecanismo de armazenamento de dados.
